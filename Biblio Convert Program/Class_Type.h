...
{
public: virtual void Start(string name_open_file, string name_save_file) = 0;
};
// implement file conversion from RIS to TXT
class from_RIS_to_TXT : public Variant_Convert
{
public: void Start(string name_open_file, string name_save_file) override;
};
...
public:
	...
	void Set_Open_File(string name_open_file);
	// save the name of the file to which you want to convert the data
	void Set_Save_File(string name_save_file);
	// start the program
